...
function App() {
  ...
  // 🚨 FIX 1: SCROLL GLITCH
  // Added 'loading' to dependencies so it scrolls when the AI starts/stops thinking.
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
    }, 100);
  ...
  // 🚨 FIX 2: ROBUST SELECTION LOGIC
  // Instead of natural language that might be ambiguous, we send a clear
  // structured sentence that the Backend Router can easily classify as "select_option".
  const handleOptionSelect = async (selectedOption) => {
    let userChoiceText = "";
    let displayText = "";
    ...
    if (selectedOption.airline) {
        // Flight Selection
        // "Select option:" acts as a trigger keyword for the LLM
        userChoiceText = `Select option: Flight ${selectedOption.number} (${selectedOption.airline})`;
        displayText = `✅ Selected: ${selectedOption.airline} Flight ${selectedOption.number}`;
    } else if (selectedOption.selected_class) {
        // Train Selection
        // We include the class name explicitly so the extraction node catches it
        userChoiceText = `Select option: Train ${selectedOption.number} (${selectedOption.name}) in class ${selectedOption.selected_class.name}`;
        displayText = `✅ Selected: ${selectedOption.name} (${selectedOption.selected_class.name})`;
    } else return;

    // 1. Show the user a pretty confirmation bubble
    setMessages(prev => [...prev, { role: 'user', content: displayText }]);
    
    // 2. Send the specific command string to the backend
    setLoading(true);
    const data = await sendMessage(userChoiceText);
    setMessages(prev => [...prev, { role: 'ai', content: data.response }]);
    setLoading(false);
